...
class RedCore {
    ...
    cloneItem(itemToClone, redCoreID) {
        if (this.mydb.items[redCoreID].enable == true) {
            ...
            const rcCompatibilities = (typeof this.mydb.items[redCoreID].rcCompatibilities == "undefined") ? {} : this.mydb.items[redCoreID].rcCompatibilities;
            const rcConflicts = (typeof this.mydb.items[redCoreID].rcConflicts == "undefined") ? [] : this.mydb.items[redCoreID].rcConflicts;
            for (const modSlotName in rcCompatibilities) {
                for (const slot of redCoreItemOut._props.Slots) {
                    if (slot._name === modSlotName)
                        for (const id of rcCompatibilities[modSlotName])
    ...
    addCompatibilitiesAndConflicts(itemClone, redCoreID) {
        for (const item in this.db.templates.items) {
            ...
            for (const entry of combined) {
                for (const id of entry._props.filters[0].Filter) {
                    if (id === itemClone)
                        entry._props.filters[0].Filter.push(redCoreID);
                }
            }
            ...
            for (const conflictID of conflictingItems)
                if (conflictID === itemClone)
                    conflictingItems.push(redCoreID);
        }
    }
